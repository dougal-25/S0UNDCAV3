...
## [2026-06-27] Foraging — manual search now actually searches keywords + genres (branch `claude/foraging-search-keywords-okv208`)

Doug flagged that manual + scheduled Foraging searches "often return no artists." Traced the whole path; the **manual** live search (`content_api.py` `/api/search`) and the **scheduled** scout (`scheduled_scout.py`) were two separate implementations and the manual one had three bugs that silently emptied results:

- **Keyword was never sent to SoundCloud.** `/api/search` only passed `genres=` to `sc_fetch_tracks`, then applied the keyword as a *local substring filter* over the ~50 hottest tracks per genre (title + username only). So any keyword that wasn't literally in those few titles returned nothing. Fix: `sc_fetch_tracks` now takes `keyword` and sends it as SoundCloud's full-text `q` query (the way `scheduled_scout.py:116` already did it). Dropped the local substring filter — `q` does the matching now.
- **Hidden 5,000-follower ceiling.** `max_followers` defaulted to `5000` server-side, and the frontend only sends the param when the user types one. Leaving "Max followers" blank therefore dropped every artist above 5k. Fix: default `0 = no ceiling`; the filter only applies a max when one is set (`if max_followers and followers > max_followers`), matching the scheduled scout.
- **Exact genre-tag matching whiffs.** SoundCloud's `genres` tag is free-text and inconsistent ("Tech House" / "tech-house" / "House" are distinct tags), so a typed genre often matched nothing. Fix: loose-genre fallback — when an exact `genres` query returns 0 tracks, retry the genre as a `q` free-text query (`genre_as_query=True`).
