...
async function generateImage(ctx) {
  ...
  try {
    ...
    if (r.status === 402) {
      const j = await r.json().catch(() => ({}));
      throw new Error(`Insufficient credits — this image costs ${j.cost || 2}.`);
    }
    if (!r.ok) {
...
async function refineImage() {
  ...
  try {
    ...
    if (r.status === 402) {
      const j = await r.json().catch(() => ({}));
      throw new Error(`Insufficient credits — refine costs ${j.cost || 2}.`);
    }
    if (!r.ok) {
      const j = await r.json().catch(() => ({}));
...
async function generateAnimation() {
  ...
  try {
    ...
    if (r.status === 402) {
      const j = await r.json().catch(() => ({}));
      throw new Error(`Insufficient credits — an animation costs ${j.cost || 4}.`);
    }
    const j = await r.json();
